...
// Validation schemas
const enrichCompanyBaseSchema = z.object({
...
type EnrichCompanySchema = z.infer<typeof enrichCompanyBaseSchema>;

const enrichCompanySchema = enrichCompanyBaseSchema.refine(
  (data: EnrichCompanySchema) => data.companyName || data.domain,
  {
    message: 'Either companyName or domain must be provided',
  }
);

const enrichBusinessCardBaseSchema = z.object({
  // Person information
...
type EnrichBusinessCardSchema = z.infer<typeof enrichBusinessCardBaseSchema>;

const enrichBusinessCardSchema = enrichBusinessCardBaseSchema.refine(
  (data: EnrichBusinessCardSchema) => data.personName || data.companyName,
  {
