...
export async function seedDemoWorkspace(options: SeedOptions = {}): Promise<void> {
  ...
  const sinoLand = await prisma.enrichmentCompanyProfile.create({
    data: {
      ...
      description: 'Leading Hong Kong property developer within the Sino Group.',
      logoUrl: 'https://cdn.namecard.app/company/sino.png',
      lastUpdated: now,
      createdAt: now,
      updatedAt: now,
    },
  ...
  const cardOne = await prisma.cardsCard.create({
    data: {
      ...
      company: sinoLand.name,
      email: 'davidng@sino.com',
      phone: '+852-2721-8388',
      address: '11 - 12/F, Tsim Sha Tsui Centre, Salisbury Road, Tsim Sha Tsui, Kowloon, Hong Kong',
      website: sinoLand.website,
      notes: 'Interested in expanding strategic partnerships across Hong Kong.',
      tags: ['priority', 'hk'],
  ...
  await prisma.enrichmentNewsArticle.create({
    data: {
      id: randomUUID(),
      companyId: sinoLand.id,
      title: 'Sino Land unveils new Kowloon redevelopment plans',
      summary: 'Sino Land accelerates sustainable redevelopment across Kowloon district.',
      url: 'https://www.sino.com/news/kowloon-development',
