...
exports.ImageValidationService = exports.AppError = void 0;
...
class ImageValidationService {
     ...
     */
    static async validateImage(buffer, originalName, config = {}) {
        ...
        try {
            lambdaLogger_1.default.debug('Starting image validation', {
                fileName: originalName,
                bufferSize: buffer.length,
            });
            // Step 1: Basic file size validation
            if (buffer.length > validationConfig.maxFileSize) {
                result.errors.push(`File size ${(buffer.length / 1024 / 1024).toFixed(2)}MB exceeds maximum allowed size of ${(validationConfig.maxFileSize / 1024 / 1024).toFixed(2)}MB`);
            }
            if (buffer.length === 0) {
            ...
            if (!validationConfig.allowedFormats.includes(metadata.format.toLowerCase())) {
                result.errors.push(`Format '${metadata.format}' not allowed. Supported formats: ${validationConfig.allowedFormats.join(', ')}`);
            }
            ...
            if (width < validationConfig.minWidth || height < validationConfig.minHeight) {
                result.errors.push(`Image dimensions ${width}x${height} are too small. Minimum required: ${validationConfig.minWidth}x${validationConfig.minHeight}`);
            }
            if (width > validationConfig.maxWidth || height > validationConfig.maxHeight) {
                result.errors.push(`Image dimensions ${width}x${height} are too large. Maximum allowed: ${validationConfig.maxWidth}x${validationConfig.maxHeight}`);
            }
            // Step 5: Aspect ratio validation
            const aspectRatio = width / height;
            if (validationConfig.maxAspectRatio && aspectRatio > validationConfig.maxAspectRatio) {
                result.errors.push(`Image aspect ratio ${aspectRatio.toFixed(2)}:1 exceeds maximum allowed ratio of ${validationConfig.maxAspectRatio}:1`);
            }
            if (validationConfig.maxAspectRatio && 1 / aspectRatio > validationConfig.maxAspectRatio) {
                result.errors.push(`Image aspect ratio 1:${(1 / aspectRatio).toFixed(2)} exceeds maximum allowed ratio of 1:${validationConfig.maxAspectRatio}`);
            }
            ...
            lambdaLogger_1.default.info('Image validation completed', {
                fileName: originalName,
            ...
            lambdaLogger_1.default.error('Image validation failed', {
                fileName: originalName,
     ...
     */
    static async validateImages(files, config = {}) {
        const validationConfig = { ...this.DEFAULT_CONFIG, ...config };
        // Check file count limit
        if (files.length > validationConfig.maxFiles) {
            throw new AppError(`Too many files. Maximum ${validationConfig.maxFiles} files allowed, received ${files.length}`, 400);
        }
        const results = [];
        let totalValid = 0;
        let totalErrors = 0;
        ...
        lambdaLogger_1.default.info('Batch image validation completed', {
     ...
     */
    static async performSecurityChecks(buffer, metadata, result, config) {
        try {
            ...
            if (config.lenientSecurity) {
                ...
                for (const pattern of suspiciousPatterns) {
                    if (headerToCheck.includes(pattern)) {
                        ...
                        lambdaLogger_1.default.debug('Suspicious pattern detected in business card image header (lenient mode)', {
                ...
                for (const pattern of suspiciousPatterns) {
                    if (headerToCheck.includes(pattern)) {
                        ...
                        lambdaLogger_1.default.warn('Suspicious pattern detected in image header', {
                            pattern: pattern.toString(),
                        });
                        break; // Only warn once
                    }
            ...
            if (metadata.format && formatSignatures[metadata.format]) {
                ...
                const matchesSignature = expectedSignatures.some(sig => buffer.subarray(0, sig.length).equals(sig) || buffer.includes(sig));
                if (!matchesSignature) {
            ...
            lambdaLogger_1.default.warn('Security check failed', {
                error: error instanceof Error ? error.message : 'Unknown error',
            });
...
ImageValidationService.DEFAULT_CONFIG = {
...
exports.default = ImageValidationService;
