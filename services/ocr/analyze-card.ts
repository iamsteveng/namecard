  ...
  try {
    ...
    if (event.headers['content-type']?.includes('multipart/form-data')) {
      try {
        ...
        logger.warn('OCR analyze-card multipart parse failed', {
    ...
    logger.error('OCR analyze-card failed', error instanceof Error ? error : undefined, {
      requestId: context.awsRequestId,
    });
    ...
    return {
      ...
      body: JSON.stringify({
        success: false,
        error: error?.message || 'Analyze card failed',
        timestamp: new Date().toISOString(),
      }),
    };
  }
