  ...
  try {
    ...
    logger.info('Image upload completed', {
      ...
      validation: {
        ...
        dimensions: validationResult.metadata ? `${validationResult.metadata.width}x${validationResult.metadata.height}` : 'unknown',
      },
    });
 ...
 * @access Private (requires JWT token)
 */
router.post('/images', authenticateToken, upload.array('images', 5), async (req: Request, res: Response) => {
  try {
    ...
    const batchValidationResult = await ImageValidationService.validateImages(fileData, validationConfig);
    ...
    const batchProcessingResults = await ImagePreprocessingService.processBatch(
      imageData,
      { purpose: 'storage' }
    );
    ...
    const processingErrors = uploadedFiles.filter(f => f.status === 'validated' && f.processing?.error).length;
